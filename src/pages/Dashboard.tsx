...
// Glow palette for "Hi There!" greeting - prevents re-renders on each component instance
const GLOW_PALETTE = {
  glowColors: ['hsl(187 100% 50% / 0.7)', 'hsl(260 80% 55% / 0.7)', 'hsl(25 100% 55% / 0.7)'],
  strokeColors: ['hsl(187 100% 70% / 0.7)', 'hsl(260 80% 70% / 0.7)', 'hsl(25 100% 70% / 0.7)'],
  backlight: 'linear-gradient(90deg, hsl(25 100% 50% / 0.4), hsl(260 80% 50% / 0.3), hsl(187 100% 50% / 0.4), hsl(330 80% 50% / 0.3), hsl(25 100% 50% / 0.4))',
  letterEffects: {
    fillTransparent: 'transparent',
    stroke: '1.5px hsl(220 20% 75% / 0.8)',
...
export default function Dashboard() {
  ...
  const { thisMonthWorkouts, totalWorkouts } = useWorkoutHistory();
          ...
          <div>
            <h1 className="text-4xl md:text-5xl font-black uppercase relative" style={{
                ...
                <span className="absolute inset-0 blur-xl opacity-60 pointer-events-none" aria-hidden="true" style={{
                  background: GLOW_PALETTE.backlight,
                ...
                {'Hi There ! '.split('').map((letter, i) => {
                  ...
                  return (
                    <span key={`ht-${i}`} className="relative inline-block text-2xl md:text-3xl font-black" style={{
                      WebkitTextFillColor: 'transparent',
                      WebkitTextStroke: GLOW_PALETTE.letterEffects.stroke,
                      filter: GLOW_PALETTE.letterEffects.filter,
                    }}>{letter}</span>
                  );
                })}
                  ...
                  return displayName.split('').map((letter, i) => {
                    ...
                    return (
                      <span key={`u-${i}`} className="relative inline-block" style={{
                        WebkitTextFillColor: 'transparent',
                        WebkitTextStroke: `1.5px ${GLOW_PALETTE.strokeColors[ci]}`,
                        filter: `drop-shadow(0 0 12px ${GLOW_PALETTE.glowColors[ci]}) drop-shadow(0 0 25px ${GLOW_PALETTE.glowColors[ci]})`,
                      }}>{letter}</span>
                    );
                  });
                })()}
